...
local function merlinRequest(command, args, bufnr)
	...
	return with_ocamllsp(function(lsp)
		...
		local res = lsp.request_sync("ocamllsp/merlinCallCompatible", params, 9000, bufnr)
		if res.err then
			return vim.api.nvim_err_writeln("ERROR: " .. vim.inspect(res.err))
		end
...
local function search_by_type(opts)
	if opts.fargs then
		opts.query = opts.fargs[1]
	end
  ...
  --]]
	...
	local function search_by_type_picker(opts)
		...
		local function ocaml_entry_maker(entry)
			local function make_display(entry)
				...
				local query =
					assert(ts_query.get("ocaml_interface", "highlights"), "Must have ocaml_interface highlights")
	...
	search_by_type_picker(opts)
end

vim.api.nvim_create_user_command("OCamlSearchByType", search_by_type, { nargs = "?" })
vim.api.nvim_create_user_command("OCamlOutline", documentSymbols, {})

vim.keymap.set("n", "<leader>os", function()
	local user_input = vim.fn.input("Search by type: ")
	searchByType({ query = user_input })
end, { desc = "[O]caml [S]earch by type" })
