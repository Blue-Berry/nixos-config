...
in
{
  ...
  config = lib.mkMerge [
    # Keep the legacy hyprlang config format (stateVersion < 26.05). Set
    # unconditionally so the changed-default warning stays silent even when
    # this module is disabled (home-manager still evaluates the option).
    {
      ...
      wayland.windowManager.hyprland = {
        ...
        settings =
          ...
          in
          {
            ...
            workspace =
              if cfg.monitorProfile == "personal" then
                [
            ...
            cursor.no_hardware_cursors = if cfg.monitorProfile == "personal" then false else true;
            ...
            ++ (
              if cfg.monitorProfile == "work" then
                [
